...
{
    // One uploaded file is capped at 10 MB (decoded). The playground replays through the in-process
    // gateway; a generous cap keeps a stray large file from buffering unbounded in memory.
    ...
    public async Task<PlaygroundResponse> ReplayAsync(PlaygroundRequest request, Uri gatewayOrigin, CancellationToken ct)
    {
        ...
        if (request.FormData is { } formData)
        {
            // Build multipart/form-data from the structured fields. HttpClient owns the boundary and sets
            // the Content-Type (so no stale multipart header is copied above): the importer carries the
            // Content-Type in BodyMediaType, never as a plain header, so the header loop never re-adds it.
            message.Content = BuildMultipartContent(formData);
        }
    ...
    // Assemble the multipart body: text fields are StringContent by name; file fields are the decoded
    // base64 bytes as ByteArrayContent by name + filename. Fails loud on bad base64 or an oversized file
    // (the endpoint maps these to a 400 so the user sees the message, not a 500).
    private static MultipartFormDataContent BuildMultipartContent(PlaygroundFormDataBody formData)
    {
        var content = new MultipartFormDataContent();
        foreach (var field in formData.Fields)
        {
            if (field.FileBase64 is { } base64)
            {
                byte[] bytes;
                try
                {
                ...
                catch (FormatException ex)
                {
                    throw new InvalidOperationException(
                        $"Form-data file field '{field.Name}' is not valid base64. Re-select the file and replay again.", ex);
                }

                if (bytes.Length > MaxFileBytes)
                {
                    throw new InvalidOperationException(
                        $"Form-data file field '{field.Name}' is {bytes.Length / (1024 * 1024)} MB; the limit is 10 MB per file.");
                }

                // The filename defaults to the field name; the model carries no separate filename today.
                content.Add(new ByteArrayContent(bytes), field.Name, field.Name);
            }
            else
            {
                content.Add(new StringContent(field.TextValue ?? ""), field.Name);
            }
        }
