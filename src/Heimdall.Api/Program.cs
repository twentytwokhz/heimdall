...
try
{
    ...
    if (builder.Configuration.GetValue("Heimdall:EnableAdminApi", false))
    {
            ...
            ILoggerFactory loggerFactory, CancellationToken ct) =>
        {
            ...
            var reloaded = await loader.LoadAsync(cfg["Heimdall:ConfigPath"] ?? "samples", ct: ct);
            if (DemoApi.IsEnabled(cfg))
            {
                reloaded = await DemoApi.MergeAsync(reloaded, loader, cfg, ct);
            }
