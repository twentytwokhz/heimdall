...
internal sealed class XmlOpenApiConfigLoader : IConfigLoader
{
    ...
    public async Task<GatewayConfig> LoadAsync(string sourcePath, CancellationToken ct = default)
    {
        var config = await ReadConfigFile(sourcePath, ct);
    ...
    private static async Task<ConfigFile> ReadConfigFile(string sourcePath, CancellationToken ct)
    {
        var path = Path.Combine(sourcePath, "config.json");
