...
public sealed class PostmanV21Importer : ICollectionImporter
{
        ...
        string name, JsonElement item, JsonElement request, IReadOnlyDictionary<string, string> variables, Uri gateway)
    {
        ...
        var (body, mediaType, formData) = ReadBody(request, variables, unresolved, notes, explicitContentType);
        ...
        return new PlaygroundRequest(name, method, url, originalUrl, headers, body, mediaType, notes, formData);
    }
    ...
    private static (string? Body, string? MediaType, PlaygroundFormDataBody? FormData) ReadBody(
        JsonElement request,
        ...
        string? explicitContentType)
    {
        ...
        {
            return (null, explicitContentType, null);
        }
        ...
        switch (GetString(body, "mode"))
        {
            case "raw":
                ...
                return (raw, explicitContentType ?? RawLanguageMediaType(body), null);

            case "urlencoded":
                return (BuildUrlEncoded(body, variables, unresolved), explicitContentType ?? "application/x-www-form-urlencoded", null);

            case "formdata":
                // Structured: the replay client assembles the multipart (and owns the boundary), so the
                // media type here is just the bare type. Body stays null; FormData carries the fields.
                return (null, "multipart/form-data", BuildFormData(body, variables, unresolved, notes));
        ...
        JsonElement body, IReadOnlyDictionary<string, string> variables, ISet<string> unresolved, List<string> notes)
    {
        ...
        if (body.TryGetProperty("formdata", out var fields) && fields.ValueKind == JsonValueKind.Array)
        {
            foreach (var field in fields.EnumerateArray())
            {
                ...
                {
                    // A file field is a slot the user fills on replay (FileBase64 set in the UI). Surface it
                    // so the dropped src path is not silently lost.
                    notes.Add($"Form-data file field '{key}' ready for upload on replay.");
                    formFields.Add(new PlaygroundFormField(key));
                    continue;
                }

                var value = PlaceholderResolver.Resolve(GetString(field, "value"), variables, unresolved);
                formFields.Add(new PlaygroundFormField(key, TextValue: value));
            }
        }

        return new PlaygroundFormDataBody(formFields);
    }
